...
func BenchmarkEJ_Marshal_M(b *testing.B) {
	var l int64
	for i := 0; i < b.N; i++ {
		data, err := partialencode.Marshal(&largeStructData)
		if err != nil {
			b.Error(err)
		}
		l = int64(len(data))
...
func BenchmarkEJ_Marshal_L(b *testing.B) {
	var l int64
	for i := 0; i < b.N; i++ {
		data, err := partialencode.Marshal(&xlStructData)
		if err != nil {
			b.Error(err)
		}
		l = int64(len(data))
...
func BenchmarkEJ_Marshal_L_ToWriter(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		w := jwriter.Writer{}
		xlStructData.MarshalPartialJSON(&w)
		if w.Error != nil {
			b.Error(w.Error)
		}
...
func BenchmarkEJ_Marshal_M_ToWriter(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		w := jwriter.Writer{}
		largeStructData.MarshalPartialJSON(&w)
		if w.Error != nil {
			b.Error(w.Error)
		}
