...
const genPackage = "github.com/reddyvinod/partialencode/gen"
const pkgWriter = "github.com/reddyvinod/partialencode/jwriter"
const pkgLexer = "github.com/reddyvinod/partialencode/jlexer"
...
func (g *Generator) writePartial() error {
	f, err := os.Create(g.PartialName)
	if err != nil {
		return err
	}
	defer f.Close()
	...
	fmt.Fprintln(f, "// TEMPORARY AUTOGENERATED FILE: partialencode stub code to make the package")
...
// writePartialMain creates a .go file that launches the generator if 'go run'.
func (g *Generator) writePartialMain() (path string, err error) {
	f, err := ioutil.TempFile(filepath.Dir(g.PartialName), "partialencode-bootstrap")
	if err != nil {
		return "", err
	}
	...
	fmt.Fprintln(f)
	fmt.Fprintln(f, "// TEMPORARY AUTOGENERATED FILE: partialencode bootstapping code to launch")
	...
	fmt.Fprintf(f, "  g := gen.NewPartialGenerator(%q)\n", filepath.Base(g.PartialName))
...
func (g *Generator) RunPartial() error {
	...
	f, err := os.Create(g.PartialName + ".tmp")
	if err != nil {
		return err
	}
	if !g.LeaveTemps {
	...
	return os.Rename(f.Name(), g.PartialName)
}
